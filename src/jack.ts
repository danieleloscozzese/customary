...
export default function preventFormSubmission(formId: string, callback?: (form: HTMLFormElement) => void): void {
  const form = relyOn(document.getElementById(formId), `There is no element with ID ${formId} in the document.`);
  ...
  if (form instanceof HTMLFormElement) {
    ...
    form.addEventListener('submit', submitHandler, <any>{ capture: true, passive: false, once: false });
  } else {
